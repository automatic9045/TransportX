...
namespace TransportX.Domains.RoadVehicles.Scripting.Commands.Chassis
{
    ...
    {
        public Brake Source { get; }

        public DynamicPart Beam { get; }
        public DynamicPart Wheel { get; }
        public Joint Hinge { get; }

        internal BrakeCommand(ScriptAvatar avatar, Brake source, DynamicPart beam, DynamicPart wheel, string jointKey)
        {
            Source = source;

            Beam = beam;
            Wheel = wheel;
            Hinge = avatar.Commander.Structure.Joints.Add(jointKey, beam, wheel, source.Motor);
        }
    }
}
