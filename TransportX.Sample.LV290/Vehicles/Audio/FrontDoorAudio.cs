...
{
    internal class FrontDoorAudio : AudioBase
    {
        private readonly BifoldDoor Door;
        ...
        public FrontDoorAudio(SoundFactory soundFactory, BifoldDoor door)
        {
            Door = door;
            OldIsOpen = Door.IsOpen;

            OpenSound = LoadSound("FrontDoorOpen.wav");
            CloseSound = LoadSound("FrontDoorClose.wav");
        ...
        public override void UpdateSound(Camera camera)
        {
            ...
            if (OpenSound.IsPlaying) OpenSound.Update(camera.Listener, camera.WorldPose.Chunk);
            if (CloseSound.IsPlaying) CloseSound.Update(camera.Listener, camera.WorldPose.Chunk);
        }
    }
}
