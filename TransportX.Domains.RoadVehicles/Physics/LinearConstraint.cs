...
namespace TransportX.Domains.RoadVehicles.Physics
{
    public class LinearConstraint : IConstraint
    {
        ...
        protected LinearConstraint(IEnumerable<Shaft> shafts)
        {
            CoefficientsKey = shafts.ToDictionary(port => port, _ => 0f);
        }

        protected LinearConstraint(params Shaft[] shafts) : this((IEnumerable<Shaft>)shafts)
        {
        }
